...
# RTL simulation 
rtl_all: rtl0 rtl0DMA rtl1 rtlJPEG rtlBMP

rtl0: | clean $(bld_dir)
	make -C $(sim_dir)/prog0/; \
	cd $(bld_dir); \
	vcs -R -sverilog $(root_dir)/$(sim_dir)/top_tb.sv -debug_access+all -full64  \
	+incdir+$(root_dir)/$(src_dir)+$(root_dir)/$(src_dir)/AXI+$(root_dir)/$(src_dir)/CPU+$(root_dir)/$(src_dir)/DMA+$(root_dir)/$(src_dir)/EPU+$(root_dir)/$(inc_dir)+$(root_dir)/$(sim_dir) \
	+define+prog0$(FSDB_DEF) \
	+prog_path=$(root_dir)/$(sim_dir)/prog0 \
  	+rdcycle=1

rtl0DMA: | clean $(bld_dir)
	make -C $(sim_dir)/prog0DMA/; \
	cd $(bld_dir); \
	vcs -R -sverilog $(root_dir)/$(sim_dir)/top_tb.sv -debug_access+all -full64  \
	+incdir+$(root_dir)/$(src_dir)+$(root_dir)/$(src_dir)/AXI+$(root_dir)/$(src_dir)/CPU+$(root_dir)/$(src_dir)/DMA+$(root_dir)/$(src_dir)/EPU+$(root_dir)/$(inc_dir)+$(root_dir)/$(sim_dir) \
	+define+prog0$(FSDB_DEF) \
	+prog_path=$(root_dir)/$(sim_dir)/prog0DMA \
  	+rdcycle=1

rtlJPEG: | clean $(bld_dir)
	make -C $(sim_dir)/prog_JPEG/; \
	cd $(bld_dir); \
	vcs -R -sverilog $(root_dir)/$(sim_dir)/top_tb_Picture.sv -debug_access+all -full64  \
	+incdir+$(root_dir)/$(src_dir)+$(root_dir)/$(src_dir)/AXI+$(root_dir)/$(src_dir)/CPU+$(root_dir)/$(src_dir)/DMA+$(root_dir)/$(src_dir)/EPU+$(root_dir)/$(inc_dir)+$(root_dir)/$(sim_dir) \
	+define+progJPEG$(FSDB_DEF) \
	+prog_path=$(root_dir)/$(sim_dir)/prog_JPEG \
  	+rdcycle=1

rtlBMP: | clean $(bld_dir)
	make -C $(sim_dir)/prog_BMP/; \
	cd $(bld_dir); \
	vcs -R -sverilog $(root_dir)/$(sim_dir)/top_tb_Picture.sv -debug_access+all -full64  \
	+incdir+$(root_dir)/$(src_dir)+$(root_dir)/$(src_dir)/AXI+$(root_dir)/$(src_dir)/CPU+$(root_dir)/$(src_dir)/DMA+$(root_dir)/$(src_dir)/EPU+$(root_dir)/$(inc_dir)+$(root_dir)/$(sim_dir) \
	+define+progBMP$(FSDB_DEF) \
	+prog_path=$(root_dir)/$(sim_dir)/prog_BMP \
  	+rdcycle=1   
...
rtl1M: | clean $(bld_dir)
	make -C $(sim_dir)/prog1M/; \
	cd $(bld_dir); \
	vcs -R -sverilog $(root_dir)/$(sim_dir)/top_tb.sv -debug_access+all -full64  \
	+incdir+$(root_dir)/$(src_dir)+$(root_dir)/$(src_dir)/AXI+$(root_dir)/$(src_dir)/CPU+$(root_dir)/$(src_dir)/DMA+$(root_dir)/$(src_dir)/EPU+$(root_dir)/$(inc_dir)+$(root_dir)/$(sim_dir) \
	+define+prog1$(FSDB_DEF) \
	+prog_path=$(root_dir)/$(sim_dir)/prog1M \
  	+rdcycle=1

rtlZZ: | clean $(bld_dir)
	cd $(bld_dir); \
	vcs -R -sverilog $(root_dir)/$(sim_dir)/tb_example/tb_zigzag.sv -debug_access+all -full64  \
	+incdir+$(root_dir)/$(sim_dir)/tb_example \
  	+rdcycle=1

rtlRLE: | clean $(bld_dir)
	cd $(bld_dir); \
	vcs -R -sverilog $(root_dir)/$(sim_dir)/tb_example/tb_RLE.sv -debug_access+all -full64  \
	+incdir+$(root_dir)/$(sim_dir)/tb_example \
  	+rdcycle=1

rtlHUFFMAN: | clean $(bld_dir)
	cd $(bld_dir); \
	vcs -R -sverilog $(root_dir)/$(sim_dir)/tb_HFF/tb_HFF.sv -debug_access+all -full64  \
	+incdir+$(root_dir)/$(sim_dir)/tb_HFF \
  	+rdcycle=1

# Post-Synthesis simulation
syn_all: syn0 syn0DMA syn1 synJPEG synBMP
...
synJPEG: | clean $(bld_dir)
	make -C $(sim_dir)/prog_JPEG/; \
	cd $(bld_dir); \
	vcs -R -sverilog +neg_tchk -negdelay -v $(lib_dir)/fsa0m_a_generic_core_21.lib.src  $(root_dir)/$(sim_dir)/top_tb_Picture.sv -debug_access+all -full64 -diag=sdf:verbose \
	+incdir+$(root_dir)/$(syn_dir)+$(root_dir)/$(inc_dir)+$(root_dir)/$(sim_dir) \
	+define+SYN+progJPEG$(FSDB_DEF) \
	+no_notifier \
	+prog_path=$(root_dir)/$(sim_dir)/prog_JPEG \
  	+rdcycle=1

synBMP: | clean $(bld_dir)
	make -C $(sim_dir)/prog_BMP/; \
	cd $(bld_dir); \
	vcs -R -sverilog +neg_tchk -negdelay -v $(lib_dir)/fsa0m_a_generic_core_21.lib.src  $(root_dir)/$(sim_dir)/top_tb_Picture.sv -debug_access+all -full64 -diag=sdf:verbose \
	+incdir+$(root_dir)/$(syn_dir)+$(root_dir)/$(inc_dir)+$(root_dir)/$(sim_dir) \
	+define+SYN+progBMP$(FSDB_DEF) \
	+no_notifier \
	+prog_path=$(root_dir)/$(sim_dir)/prog_BMP \
  	+rdcycle=1

# Post-Layout simulation
pr_all: pr0 pr0DMA pr1 prJPEG prBMP
...
prJPEG: | clean $(bld_dir)
	make -C $(sim_dir)/prog_JPEG/; \
	cd $(bld_dir); \
	vcs -R -sverilog +neg_tchk -negdelay -v $(lib_dir)/fsa0m_a_generic_core_21.lib.src  $(root_dir)/$(sim_dir)/top_tb_Picture.sv -debug_access+all -full64 -diag=sdf:verbose \
	+incdir+$(root_dir)/$(pr_dir)+$(root_dir)/$(inc_dir)+$(root_dir)/$(sim_dir) \
	+define+PR+progJPEG$(FSDB_DEF) \
	+no_notifier \
	+maxdelays \
	+prog_path=$(root_dir)/$(sim_dir)/prog_JPEG \
...
clean:
	...
	make -C $(sim_dir)/prog_JPEG/ clean; \
